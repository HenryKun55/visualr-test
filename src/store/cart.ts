...
export const useCartStore = create<State & Actions>((set) => ({
  ...
  addItem: (item) => {
    return set((state) => {
      ...
      if (theItem) {
        if (!theItem.quantity) theItem.quantity = 1;
        else if (theItem.quantity >= 99) theItem.quantity = 99;
  ...
  editItem: (_item) =>
    set((state) => {
      const items = state.items.map((item) =>
        item.id === _item.id ? { ...item, ..._item } : item
      );
      return { ...state, items, total: calculateTotal(items) };
    }),
  togglePayment: () => set((state) => ({ ...state, payment: !state.payment })),
  handleMethod: (paymentForm) =>
    set((state) => ({ ...state, paymentForm: paymentForm })),
}));
